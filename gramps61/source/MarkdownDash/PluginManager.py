...
class PluginStatus(tool.Tool, ManagedWindow):
    ...
    def __init__(self, dbstate, uistate, track):
        ...
        self._show_available = None   # show uninstalled/available plugins
        self._show_addons = None      # show •addon (3rd party) rows
        self.addons = []
        ...
        # count label LEFT of the search box: "Showing x of y plugins"
        ...
        # filter input box (to the right of the count label)
        self.filter_entry = Gtk.SearchEntry()
        self.filter_entry.set_tooltip_text(
            _('Filter plugins by entering search terms.\n'
              '\n'
    ...
    def _update_count_label(self):
        ...
        Always displayed; total = all rows that passed the show-flags checkboxes,
        visible = those that also pass the current text filter.
        """
    ...
    def _apply_filter(self, model, tr_iter, _data):
        """
        Row visibility function for self._tree_filter.

        Two search modes, selected by the query text:

        Exact-phrase mode  — query is wrapped in double-quotes:
            "person card"  →  row must contain that exact substring
                              (case-insensitive).

        All-words mode (default)  — bare words:
            person card    →  every word must appear somewhere in the
                              combined row text (case-insensitive, any order).

        All visible and diagnostic fields are included in the search corpus:
        type, status, name, description, plugin-id, filename, path, authors,
        author emails, version, gramps_target_version, category, viewclass,
        load_on_reg, require_active, tool_modes, report_modes, optionclass,
        requires_mod, requires_gi, requires_exe, help_url.
        """
        raw = self.filter_entry.get_text()
        if not raw:
            return True

        # ── Detect exact-phrase mode ───────────────────────────────────────
        stripped = raw.strip()
    ...
    def _row_search_text(self, model, tr_iter):
        """
        ...
        # All pdata attributes not already in the model columns
        pdata = self._preg.get_plugin(pid)
        if pdata:
            _str = str   # local alias for speed
            for val in (
                pdata.fname,
                pdata.fpath,
                pdata.version,
    ...
    def __info(self, pid):
        ...
        # ── Extended / diagnostic fields (only when pdata is available) ────
        if pdata:
            gtv = getattr(pdata, 'gramps_target_version', None)
            if gtv:
                self._bufin(_("Target Gramps"), str(gtv) + '\n')
            lor = getattr(pdata, 'load_on_reg', None)
            if lor:
                self._bufin(_("Load on register"), str(lor) + '\n')
            cat = getattr(pdata, 'category', None)
            if cat:
                self._bufin(_("Category"), str(cat) + '\n')
            vc = getattr(pdata, 'viewclass', None)
            if vc:
                self._bufin(_("View class"), str(vc) + '\n')
            oc = getattr(pdata, 'optionclass', None)
            if oc:
                self._bufin(_("Option class"), str(oc) + '\n')
            ra = getattr(pdata, 'require_active', None)
            if ra is not None and ra is not False:
                self._bufin(_("Require active"), str(ra) + '\n')
            rm = getattr(pdata, 'report_modes', None)
            if rm:
                self._bufin(_("Report modes"), str(rm) + '\n')
            tm = getattr(pdata, 'tool_modes', None)
            if tm:
                self._bufin(_("Tool modes"), str(tm) + '\n')
        self.helpname = ""
        if not self.help:
        ...
        # ── Diagnostics block ─────────────────────────────────────────────
        # Shows raw registration data useful for debugging APS scan issues.
        # ptype integer, success-list membership, fail-list membership,
        # and whether pdata was found in PluginRegister vs only in GuiPluginManager.
        self._bufin('\n' + _('Diagnostics'), '\n', color="#888888")
        if pdata:
            self._bufin(_('  ptype'), '{} ({})\n'.format(
                pdata.ptype, PTYPE_STR.get(pdata.ptype, '?')))
            in_success = any(pdata.id == e[2].id for e in success_list)
            self._bufin(_('  In success_list'),
                        '{}\n'.format(in_success),
                        color="green" if in_success else None)
            in_fail = any(pdata == e[2] for e in fail_list)
            if in_fail:
                self._bufin(_('  In fail_list'), 'YES\n', color="red")
            preg_pdata = self._preg.get_plugin(pid)
            self._bufin(_('  In PluginRegister'),
                        '{}\n'.format(preg_pdata is not None))
            self._bufin(_('  fpath'),
                        (pdata.fpath or '(none)') + '\n')
            self._bufin(_('  fname'),
                        (pdata.fname or '(none)') + '\n')
        else:
            self._bufin(_('  pdata'), 'NOT in PluginRegister\n', color="red")
            self._bufin(_('  Source'), 'Online addons list only\n')
        return
    ...
    def __populate_reg_list(self):
        ...
        for plugin_dict in self.addons:
            ...
            else:  # new addon — available online but not installed
        ...
        for (_type, typestr) in PTYPE_STR.items():
            for pdata in self._preg.type_plugins(_type):
                ...
                if 'gramps/plugins' in pdata.fpath.replace('\\', '/'):
                    ...
                    # Third-party installed addon — use bullet marker
...
class PluginManagerOptions(tool.ToolOptions):
    ...
    def __init__(self, name, person_id=None):
        ...
        self.options_dict = {
            'show_hidden'    : True,
            'show_builtins'  : True,
            'show_available' : False,   # uninstalled/available plugins hidden by default
            'show_addons'    : True,    # third-party installed addons shown by default
            'pane_setting'   : 400
        }
        self.options_help = {
            ...
            'show_builtins': ("=0/1", "Show builtin Plugins",
                              ...
                              True),
            'show_available': ("=0/1", "Show uninstalled/available Plugins",
                               ["Do not show uninstalled Plugins",
                                "Show uninstalled Plugins"],
                               False),
            'show_addons': ("=0/1", "Show third-party addon Plugins",
                            ["Do not show addon Plugins",
                             "Show addon Plugins"],
                            True),
