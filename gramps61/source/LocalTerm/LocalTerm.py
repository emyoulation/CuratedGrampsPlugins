...
# 2026-02-20
# - Single-language mode: when Lang1 == Lang2, show four Weblate link columns:
#     Review (translate editor exact source), Search (browse contains source),
#     Filter (projects query editor), Translated search (browse contains target)
# - URL generation via _weblate_urls(); lang code via _lang_code_from_filename()
# - Columns hidden when two different languages are selected
# - act() dispatches to Weblate URL or glossary anchor based on clicked column
# - Config schema bumped to 0.3.0
#
# ----------------------------------------------------------------------
...
COL_FG          = 6
COL_BG          = 7
# Weblate search link columns (single-language mode only)
# Even indices = URL stored in model (used for click dispatch, never shown)
# Odd indices  = human-readable label shown in the cell
COL_WL_REVIEW   = 8   # URL:   translate editor exact-source search
COL_WL_BROWSE   = 9   # URL:   browse contains-source search
COL_WL_FILTER   = 10  # URL:   projects query-editor filter
COL_WL_TRANS    = 11  # URL:   browse contains-translated search
# Display labels (what the cell actually renders)
COL_WL_LBL_REVIEW = 12  # e.g.  "Home Person"
COL_WL_LBL_BROWSE = 13  # e.g.  *Home* *Person*
COL_WL_LBL_FILTER = 14  # e.g.  ? Home Person
COL_WL_LBL_TRANS  = 15  # e.g.  *Hauptperson*
...
def _lang_code_from_filename(filename):
    """Extract the BCP-47-ish language code from a glossary CSV filename.
    Returns None for non-language files like en(original) or aa_rev1."""
...
def _weblate_urls(term, translated, lang_code):
    ...
    Returns (review_url, browse_url, filter_url, trans_url,
             lbl_review, lbl_browse, lbl_filter, lbl_trans).
    Any URL/label that cannot be built (empty inputs) is returned as ''.

    Labels match what Weblate itself shows in its search bar:
      Review  : "Term"               (exact quoted source)
      Search  : *Word* *Word*        (word wildcards)
      Filter  : ? Term               (query-editor prefix hint)
      Translated: *Translation*      (wildcard on translated value)
    """
    if not lang_code:
        return ('', '', '', '')
    ...
    # 1. Review: translate editor exact source match  (+ for spaces)
    if term:
        q1 = urllib.parse.urlencode({'q': 'source:="{}"'.format(term)})
        url1 = ('{}/translate/{}/{}/{}/?{}'
                '&sort_by=-priority%2Cposition&checksum=').format(
                    base, proj, comp, lang, q1)
    else:
        url1 = ''

    # 2. Browse: source contains  (+ for spaces)
    if term:
    ...
    # 3. Filter: projects query editor  (%20 for spaces, %22 for quotes)
    if term:
    ...
    # 4. Translated: browse contains translated term  (+ for spaces)
    if translated:
    ...
    # Review:  "Term"   (quoted, same as Weblate's exact-source display)
    lbl1 = '\u201c{}\u201d'.format(term) if term else ''   # "Term"

    # Search:  *Word* *Word*  (one wildcard token per word)
    if term:
        lbl2 = ' '.join('*{}*'.format(w) for w in term.split())
    else:
        lbl2 = ''

    # Filter:  ? Term   (mirrors Weblate query-editor prompt style)
    lbl3 = '\u2753 {}'.format(term) if term else ''         # ❓ Term
...
class LocalTerm(Gramplet):
    ...
    def main(self):
        ...
        # Single-language mode: lang1 == lang2
        self._single_lang_mode = (self.__lang1 == self.__lang2)

        # Extract Weblate language code from the lang1 filename
        if self.__files:
            self._lang1_code = _lang_code_from_filename(
                self.__files[self.__lang1])
        else:
            self._lang1_code = None

        # Column visibility
        # col 2 = Language 2 (hidden in single-lang mode)
        self.gui.WIDGET.get_column(2).set_visible(not self._single_lang_mode)
        # col 3 = Anchor (configurable)
        ...
        # Weblate link columns (cols 4-7 within treeview col list,
        # after anchor):  show only in single-lang mode with a valid lang code
        _show_wl = self._single_lang_mode and bool(self._lang1_code)
        for _col in (self._col_wl_review, self._col_wl_browse,
                     self._col_wl_filter, self._col_wl_trans):
        ...
        for row_key, value in self.lang1_txt.items():
            ...
            lbl_review = lbl_browse = lbl_filter = lbl_trans = ''
            if self._single_lang_mode and self._lang1_code:
            ...
            self.model.append([
                base_source,                           # COL_TERM           0
                value,                                 # COL_TARGET         1
                self.lang2_txt.get(base_source, ""),   # COL_LANG2          2
                self.lang1_trcom.get(row_key, ""),     # COL_TR_COM_TGT     3
                self.lang2_trcom.get(base_source, ""), # COL_TR_COM_L2      4
                self.lang1_loc.get(row_key, ""),       # COL_ANCHOR         5
                fg,                                    # COL_FG             6
                bg,                                    # COL_BG             7
                wl_review,                             # COL_WL_REVIEW      8
                wl_browse,                             # COL_WL_BROWSE      9
                wl_filter,                             # COL_WL_FILTER      10
                wl_trans,                              # COL_WL_TRANS       11
                lbl_review,                            # COL_WL_LBL_REVIEW  12
                lbl_browse,                            # COL_WL_LBL_BROWSE  13
                lbl_filter,                            # COL_WL_LBL_FILTER  14
                lbl_trans,                             # COL_WL_LBL_TRANS   15
            ])

        # Apply endonyms to headers
        columns = self.gui.WIDGET.get_columns()

        lang1_name = self.lang1_endonym or _("Target")
        columns[1].set_title(lang1_name)
        columns[2].set_title(self.lang2_endonym or _("Language 2"))

        # Label Weblate columns with the language name for clarity
        if self._single_lang_mode and self._lang1_code:
        ...
        if self._single_lang_mode and self._lang1_code:
            self.set_tooltip(_(
                "Single-language mode: double-click Review/Search/Filter/"
                "Translated search to open Weblate for that term."
            ))
        else:
    ...
    def act(self, _tree_view, path, column):
        tree_iter = self.model.get_iter(path)

        # Check if a Weblate link column was activated.
        # Each Weblate column carries ._wl_url_col = the model index of its URL.
    ...
    def build_gui(self):
        self.model = Gtk.ListStore(
            ...
            str,  # COL_TARGET         1
            str,  # COL_LANG2          2
            str,  # COL_TR_COM_TGT     3
            str,  # COL_TR_COM_L2      4
            str,  # COL_ANCHOR         5
            str,  # COL_FG             6
            str,  # COL_BG             7
            str,  # COL_WL_REVIEW      8  (URL, not shown directly)
            str,  # COL_WL_BROWSE      9  (URL, not shown directly)
            str,  # COL_WL_FILTER      10 (URL, not shown directly)
            str,  # COL_WL_TRANS       11 (URL, not shown directly)
            str,  # COL_WL_LBL_REVIEW  12 (display label)
            str,  # COL_WL_LBL_BROWSE  13 (display label)
            str,  # COL_WL_LBL_FILTER  14 (display label)
            str,  # COL_WL_LBL_TRANS   15 (display label)
        )
        ...
        # ── Weblate link columns (single-language mode only) ──────────────
        # Each column stores a URL; clicking opens it via act_weblate().
        # Columns are hidden when two different languages are selected.
        renderer_link = Gtk.CellRendererText()
        renderer_link.set_property("foreground", "#0055cc")
        renderer_link.set_property("underline", 1)   # PANGO_UNDERLINE_SINGLE

        def _wl_col(title, url_col_idx, lbl_col_idx):
            """Create a Weblate link column.

            url_col_idx  -- model column holding the URL (used in act(), never
                            bound to the renderer's text property)
            lbl_col_idx  -- model column holding the human-readable label that
                            is actually rendered in the cell
            """
            col = Gtk.TreeViewColumn(title, renderer_link,
                                     text=lbl_col_idx,      # show label, not URL
                                     foreground=COL_FG,
                                     background=COL_BG)
            col.set_visible(False)   # shown only in single-lang mode
            # Store url_col_idx on the column object so act() can fetch the URL
    ...
    def copy_selected_row(self, menu_item):
        ...
        tab-separated for easy pasting into editors/spreadsheets.

        Normal (two-language) mode columns:
          Term | Target | Language2 | Anchor-URL

        Single-language mode columns (Lang1 == Lang2):
          Term | Target | Anchor-URL | Review-URL | Search-URL | Filter-URL | Translated-search-URL
        """
        treeview = getattr(self, "treeview", None)
        if treeview is None:
            return
        ...
        # Compose anchor URL (same logic as act())
        raw_anchor = (model.get_value(tree_iter, COL_ANCHOR) or "").strip()
        if raw_anchor:
            if raw_anchor.startswith(("https://", "http://")):
                anchor_url = raw_anchor
        ...
        if getattr(self, '_single_lang_mode', False):
            # Single-language mode: include all four Weblate URLs
            cols = [
