...
class PluginStatus(tool.Tool, ManagedWindow):
    ...
    def __init__(self, dbstate, uistate, track):
        ...
        # count label: shows "showing x of y" — updated after every list rebuild
        self._count_label = Gtk.Label()
        self._count_label.set_markup('<small></small>')
    ...
    def _update_count_label(self):
        ...
        total  = rows in the underlying model (all that passed show-flags)
        visible = rows that also pass the current text filter
        """
        ...
        if total == 0:
            ...
            self._count_label.set_markup(
                '<small>{} plugins</small>'.format(total))
        else:
            self._count_label.set_markup(
                '<small>showing {} of {}</small>'.format(visible, total))
